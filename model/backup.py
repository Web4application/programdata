...
from model.common_layer import share_embedding, get_input_from_batch, get_output_from_batch, sequence_mask
import random
...
class SeqToSeq(nn.Module):
    ...
    def __init__(self, vocab, model_file_path=None, is_eval=False):
        ...
        encoder = Encoder(self.vocab_size, self.embedding)
        decoder = Decoder(self.vocab_size, self.embedding)
    ...
    def train_one_batch(self, batch, n_iter, train=True):
        ...
        if(train):
            ...
            nn.utils.clip_grad_norm_(self.encoder.parameters(), config.max_grad_norm)
            nn.utils.clip_grad_norm_(self.decoder.parameters(), config.max_grad_norm)
            nn.utils.clip_grad_norm_(self.reduce_state.parameters(), config.max_grad_norm)

            self.optimizer.step()

        return loss.item(), math.exp(loss.item()), 0,loss
